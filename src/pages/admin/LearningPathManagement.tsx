  ...
  return (
    <div>
      <div className="flex justify-between items-center mb-6">
        <div>
          <h1 className="text-2xl font-bold text-stone-900">Learning Path Management</h1>
          <p className="text-sm text-stone-500 mt-1">Manage platform learning paths</p>
        </div>
      </div>
        ...
        <table className="w-full text-sm">
          <thead>
            <tr className="border-b bg-stone-50">
              ...
              <th className="px-4 py-3 text-left font-medium text-stone-600">Users</th>
          ...
          <tbody>
            {loading ? (
              ...
              paths.map((path) => (
                <tr key={path.id} className="border-b last:border-0 hover:bg-stone-50">
                  <td className="px-4 py-3">
                    <div className="font-medium text-stone-900 max-w-xs truncate">{path.title}</div>
                  </td>
                  <td className="px-4 py-3 text-stone-600">{path.category_name || '-'}</td>
                  <td className="px-4 py-3">
                    <span
                      className={`inline-flex items-center px-2 py-0.5 rounded text-xs font-medium ${
                        path.is_public ? 'bg-emerald-100 text-emerald-700' : 'bg-stone-100 text-stone-700'
                      }`}
                    >
                  ...
                  <td className="px-4 py-3">
                    <span
                      className={`inline-flex items-center px-2 py-0.5 rounded text-xs font-medium ${
                        path.is_active ? 'bg-emerald-100 text-emerald-700' : 'bg-red-100 text-red-700'
                      }`}
                    >
                      {path.is_active ? 'Active' : 'Inactive'}
                    </span>
                  </td>
                  <td className="px-4 py-3 text-stone-600">{path.user_count.toLocaleString()}</td>
                  <td className="px-4 py-3 text-stone-500 text-xs">{formatDate(path.created_at)}</td>
                  <td className="px-4 py-3">
                    <Button
                      size="sm"
                      variant="ghost"
