...
async function copySummary(result: AiPathGenerateResponse | null, intro: string) {
  if (!result) return
...
export default function AIPathDetail() {
  ...
  const totalSubNodes = result?.data.nodes.reduce((sum, node) => sum + (node.sub_nodes?.length || 0), 0) || 0
  const totalResources = result?.data.nodes.reduce(
    (sum, node) =>
      sum +
      collectNodeResources(node).length +
      (node.sub_nodes || []).reduce((subSum, subNode) => subSum + (subNode.resources?.length || 0), 0),
    0
  ) || 0
          ...
          <div className="flex flex-col gap-8 lg:flex-row lg:items-end lg:justify-between">
            <div className="max-w-4xl">
              <div className="mb-4 flex items-center gap-2">
                ...
                <span className="text-[10px] font-bold uppercase tracking-[0.25em] text-stone-400">Learning Guide</span>
              </div>
        ...
        {!result ? (
          <div className="rounded-md border border-dashed border-stone-300 bg-white px-6 py-20 text-center">
            <div className="mx-auto max-w-xl">
              <h2 className="text-2xl font-black tracking-tight text-stone-900">还没有 AI Path 结果</h2>
          ...
          <>
            {result.warnings?.length ? (
              <section className="mb-8 rounded-md border border-amber-200 bg-amber-50 px-5 py-5 md:px-6">
                <p className="text-[10px] font-bold uppercase tracking-[0.25em] text-amber-600">Reading Notes</p>
            ...
            <div className="grid grid-cols-1 gap-8 lg:grid-cols-12 lg:items-start">
              <article className="lg:col-span-8 space-y-8">
                <section className="rounded-md border border-stone-200 bg-white px-6 py-6 shadow-sm md:px-8 md:py-8">
                  <p className="text-[10px] font-bold uppercase tracking-[0.25em] text-stone-400">How to use this guide</p>
                      ...
                      <div className="flex flex-col gap-4 md:flex-row md:items-start md:justify-between">
                        ...
                        <div className="grid grid-cols-2 gap-3 text-xs text-stone-500 md:min-w-48">
                          <div className="rounded-md border border-stone-200 bg-white px-4 py-3">
                            <div className="font-bold text-stone-900">{node.sub_nodes?.length || 0}</div>
                            <div className="mt-1">sub topics</div>
                          </div>
                          <div className="rounded-md border border-stone-200 bg-white px-4 py-3">
                            <div className="font-bold text-stone-900">{collectAllNodeResources(node).length}</div>
                      ...
                      <div className="space-y-8">
                        {node.explanation && (
                          <section className="space-y-3">
                            <p className="text-[10px] font-bold uppercase tracking-[0.25em] text-stone-400">Stage overview</p>
                                  ...
                                  <div className="flex flex-col gap-4 lg:flex-row lg:items-start lg:justify-between">
                                    <div className="min-w-0 flex-1">
                                      <div className="flex items-center gap-3">
                                        ...
                                        <h3 className="text-lg font-bold text-stone-900">{subNode.title}</h3>
                                      </div>
                                      {subNode.description && (
                                        <p className="mt-3 text-sm leading-7 text-stone-600">{subNode.description}</p>
                                      )}

                                      {subNode.learning_points?.length ? (
                                        <ul className="mt-4 space-y-2">
                                          {subNode.learning_points.map((point: string, pointIdx: number) => (
                                            <li
                                      ...
                                      <aside className="w-full lg:w-80">
                                        <div className="rounded-md border border-stone-200 bg-white p-4">
                                          ...
                                          <div className="mt-4 space-y-3">
                                            {subNode.resources.map((resource) => (
                                              <a
                                              ...
                                              >
                                                <div
                                                  ...
                                                  style={{ backgroundImage: `url(${resourceThumbnail(resource.url)})` }}
                                                />
                                                <div className="min-w-0">
                                                  <div
                                                    className="text-[10px] font-semibold uppercase tracking-wider"
                                                    style={{ color: getCategoryColor(resourceHost(resource.url)) }}
                                                  >
                              ...
                              {collectNodeResources(node).map((resource) => (
                                ...
                                >
                                  ...
                                  <div className="p-4">
                                    ...
                                    <p className="mt-2 line-clamp-2 text-xs leading-5 text-stone-400">{resource.url}</p>
                                  </div>
                                </a>
                              ))}
                            </div>
                ...
                {result.data.recommendations?.length ? (
                  <section className="rounded-md border border-stone-200 bg-white px-6 py-6 shadow-sm md:px-8 md:py-8">
                    <p className="text-[10px] font-bold uppercase tracking-[0.25em] text-stone-400">Final recommendations</p>
                    <div className="mt-4 space-y-3">
                      {result.data.recommendations.map((item: string, idx: number) => (
                        <div
                          key={`${item}-${idx}`}
              ...
              <aside className="lg:col-span-4 lg:sticky lg:top-24 space-y-5">
                <section className="rounded-md border border-stone-200 bg-white p-5 shadow-sm">
                  <p className="text-[10px] font-bold uppercase tracking-[0.25em] text-stone-400">At a glance</p>
                  <div className="mt-4 grid grid-cols-1 gap-3 sm:grid-cols-3 lg:grid-cols-1">
                    <div className="rounded-md bg-stone-50 px-4 py-4">
                      <div className="text-2xl font-black tracking-tight text-stone-900">{result.data.nodes.length}</div>
                      <div className="mt-1 text-xs text-stone-500">stages</div>
                    </div>
                    <div className="rounded-md bg-stone-50 px-4 py-4">
                      <div className="text-2xl font-black tracking-tight text-stone-900">{totalSubNodes}</div>
                      <div className="mt-1 text-xs text-stone-500">sub topics</div>
                    </div>
                    <div className="rounded-md bg-stone-50 px-4 py-4">
                      <div className="text-2xl font-black tracking-tight text-stone-900">{totalResources}</div>
                ...
                <section className="rounded-md border border-stone-200 bg-white p-5 shadow-sm">
                  <p className="text-[10px] font-bold uppercase tracking-[0.25em] text-stone-400">Outline</p>
                  <nav className="mt-4 space-y-2">
                    {result.data.nodes.map((node, idx) => (
                      ...
                      >
                        ...
                        <div className="mt-1 text-sm font-semibold leading-6 text-stone-800">{node.title}</div>
                      </a>
                    ))}
                  </nav>
                </section>
                ...
                <section className="rounded-md border border-stone-200 bg-white p-5 shadow-sm">
                  <p className="text-[10px] font-bold uppercase tracking-[0.25em] text-stone-400">Reading strategy</p>
