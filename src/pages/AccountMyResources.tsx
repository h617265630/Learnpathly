...
function mapDb(r: DbResource): UiResource {
  ...
  return {
    ...
    thumbnail: String(anyR.thumbnail || anyR.thumbnail_url || '').trim() || FALLBACK_THUMB,
  }
}
  ...
  if (filtered.length === 0) {
    return (
      <div className="py-16 text-center bg-white rounded-md border border-stone-100">
        ...
        <h3 className="text-sm font-bold text-stone-700 mb-1">No resources yet</h3>
        <p className="text-xs text-stone-400 mb-5">Start building your personal knowledge library.</p>
        <button
