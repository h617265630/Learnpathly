...
export default function AccountUserInfo() {
  const { user, isAuthed, fetchProfile, bumpAvatarBuster, avatarBuster } = useAuthStore()
  ...
  const avatarSrc = avatarUrl ? `${avatarUrl}${avatarUrl.includes('?') ? '&' : '?'}v=${avatarBuster}` : null
  ...
  const uploadAvatar = async () => {
    ...
    try {
      ...
      setError(String(e?.response?.data?.detail || e?.message || 'Failed to upload avatar'))
    } finally {
  ...
  return (
    <div>
      {/* Section header */}
      <div className="mb-6">
        <p className="text-[10px] font-bold uppercase tracking-[0.3em] text-stone-400 mb-1.5">Account</p>
        <h2 className="text-2xl font-black text-stone-900 font-serif tracking-tight">User Info.</h2>
      </div>
      ...
      <div className="mb-8 p-6 bg-white rounded-md border border-stone-100 shadow-sm">
        <div className="flex items-center gap-4 mb-5">
          <div className="w-16 h-16 shrink-0 overflow-hidden rounded-none border-2 border-stone-100">
            {avatarSrc ? (
              <img src={avatarSrc} alt={displayName} referrerPolicy="no-referrer" className="h-full w-full rounded-none object-cover" />
            ) : (
          ...
          <div className="min-w-0">
            <p className="text-base font-bold text-stone-900 truncate">{displayName}</p>
        ...
        <div className="grid gap-3 sm:grid-cols-2">
          <div className="px-4 py-3 bg-stone-50 rounded-sm border border-stone-100">
            <p className="text-[10px] font-bold uppercase tracking-[0.15em] text-stone-400 mb-1">Username</p>
            <p className="text-sm font-semibold text-stone-900 break-all">{username || '—'}</p>
          </div>
          <div className="px-4 py-3 bg-stone-50 rounded-sm border border-stone-100">
            <p className="text-[10px] font-bold uppercase tracking-[0.15em] text-stone-400 mb-1">Email</p>
            <p className="text-sm font-semibold text-stone-900 break-all">{email || '—'}</p>
          </div>
        </div>
      </div>
        ...
        <div className="space-y-5">
          ...
          <div>
            ...
            <div className="flex flex-col sm:flex-row gap-3 sm:items-center">
              ...
              {pickedFileName && (
                <p className="text-xs text-stone-400 break-all">{pickedFileName}</p>
              )}
            </div>
          </div>
