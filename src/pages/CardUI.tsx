...
const allStyles: { style: string; variants: { variant: string; color: string; bg: string }[] }[] = [
  { style: 'tier', variants: [
    { variant: 'gold', color: '#ca8a04', bg: '' }, { variant: 'diamond', color: '#0891b2', bg: '' },
    { variant: 'prismatic', color: '#7c3aed', bg: '' }, { variant: 'obsidian', color: '#18181b', bg: '' },
    { variant: 'silver', color: '#71717a', bg: '' }, { variant: 'bronze', color: '#d97706', bg: '' },
  ]},
  { style: 'gradient', variants: [
    { variant: 'emerald', color: '#10b981', bg: '' }, { variant: 'sapphire', color: '#3b82f6', bg: '' },
    { variant: 'ruby', color: '#ef4444', bg: '' }, { variant: 'amethyst', color: '#8b5cf6', bg: '' },
    { variant: 'gold', color: '#eab308', bg: '' },
  ]},
  { style: 'glass', variants: [
  ...
  { style: 'neon', variants: [
    { variant: 'cyan', color: '#00FFFF', bg: '' }, { variant: 'pink', color: '#FF006E', bg: '' },
    { variant: 'green', color: '#39FF14', bg: '' }, { variant: 'purple', color: '#BF00FF', bg: '' },
    { variant: 'gold', color: '#FFD700', bg: '' },
  ]},
  { style: 'brutalist', variants: [
    { variant: 'red', color: '#FF0000', bg: '' }, { variant: 'blue', color: '#0000FF', bg: '' },
    { variant: 'yellow', color: '#FFD700', bg: '' }, { variant: 'green', color: '#00CC00', bg: '' },
  ]},
  { style: 'aurora', variants: [
  ...
  { style: 'retro', variants: [
    { variant: 'green', color: '#39FF14', bg: '' }, { variant: 'amber', color: '#FFB000', bg: '' },
    { variant: 'cyan', color: '#00FFFF', bg: '' }, { variant: 'red', color: '#FF3333', bg: '' },
    { variant: 'white', color: '#E0E0E0', bg: '' },
  ]},
  { style: 'watercolor', variants: [
    { variant: 'rose', color: '#e91e63', bg: '' }, { variant: 'violet', color: '#9c27b0', bg: '' },
    { variant: 'ocean', color: '#2196f3', bg: '' }, { variant: 'amber', color: '#ff9800', bg: '' },
    { variant: 'teal', color: '#009688', bg: '' },
  ]},
  { style: 'mesh', variants: [
    { variant: 'sunset', color: '#e879f9', bg: '' }, { variant: 'forest', color: '#34d399', bg: '' },
    { variant: 'fire', color: '#f43f5e', bg: '' }, { variant: 'arctic', color: '#67e8f9', bg: '' },
    { variant: 'candy', color: '#fb7185', bg: '' },
  ]},
  { style: 'sketch', variants: [{ variant: 'default', color: '', bg: '' }] },
  { style: 'metallic', variants: [
    { variant: 'steel', color: '#78909c', bg: '' }, { variant: 'copper', color: '#b87333', bg: '' },
    { variant: 'titanium', color: '#607d8b', bg: '' }, { variant: 'rose-gold', color: '#c47f7f', bg: '' },
    { variant: 'gunmetal', color: '#6b7b83', bg: '' },
  ]},
  { style: 'pixel', variants: [
    { variant: 'red', color: '#FF6B6B', bg: '' }, { variant: 'teal', color: '#4ECDC4', bg: '' },
    { variant: 'yellow', color: '#FFE66D', bg: '' }, { variant: 'purple', color: '#A78BFA', bg: '' },
    { variant: 'green', color: '#34D399', bg: '' },
  ]},
  { style: 'papercut', variants: [
    { variant: 'coral', color: '#FF6B6B', bg: '' }, { variant: 'sky', color: '#4FC3F7', bg: '' },
    { variant: 'mint', color: '#66BB6A', bg: '' }, { variant: 'lavender', color: '#AB47BC', bg: '' },
    { variant: 'peach', color: '#FF8A65', bg: '' },
  ]},
  { style: 'vaporwave', variants: [
    { variant: 'pink', color: '#ff71ce', bg: '' }, { variant: 'cyan', color: '#01cdfe', bg: '' },
    { variant: 'green', color: '#05ffa1', bg: '' }, { variant: 'purple', color: '#b967ff', bg: '' },
    { variant: 'yellow', color: '#fffb96', bg: '' },
  ]},
  { style: 'newspaper', variants: [{ variant: 'default', color: '#1a1a1a', bg: '' }] },
  { style: 'frosted', variants: [
    { variant: 'indigo', color: '#6366f1', bg: '' }, { variant: 'pink', color: '#ec4899', bg: '' },
    { variant: 'teal', color: '#14b8a6', bg: '' }, { variant: 'amber', color: '#f59e0b', bg: '' },
    { variant: 'violet', color: '#8b5cf6', bg: '' },
  ]},
  { style: 'origami', variants: [
    { variant: 'red', color: '#e74c3c', bg: '' }, { variant: 'blue', color: '#3498db', bg: '' },
    { variant: 'green', color: '#2ecc71', bg: '' }, { variant: 'orange', color: '#f39c12', bg: '' },
    { variant: 'purple', color: '#9b59b6', bg: '' },
  ]},
  { style: 'glitch', variants: [
    { variant: 'matrix', color: '#00ff41', bg: '' }, { variant: 'danger', color: '#ff0040', bg: '' },
    { variant: 'ice', color: '#00d4ff', bg: '' }, { variant: 'electric', color: '#fffc00', bg: '' },
...
function assignRandomStyles(resources: DbResource[]): Record<number, CardStyleEntry> {
...
function tierAccent(weight: string): string {
  const map: Record<string, string> = {
    default: '#78716c', iron: '#475569', bronze: '#d97706', silver: '#71717a',
...
function metallicTextMain(type: string): string {
  const m: Record<string, string> = {
    steel: '#e0e0e0', copper: '#f5d5b8', titanium: '#cfd8dc', 'rose-gold': '#f5d5d5', gunmetal: '#c0c8cc'
  }
...
function metallicTextSub(type: string): string {
  const m: Record<string, string> = {
    steel: '#90a4ae', copper: '#b87333aa', titanium: '#78909c', 'rose-gold': '#c47f7faa', gunmetal: '#78909c'
  }
  ...
  return (
    <div className={`h-full flex flex-col overflow-hidden ${className}`}>
      <div className="px-3 py-2 flex items-center justify-between border-b border-black/10">
        <span className="text-xs font-bold uppercase tracking-wider text-stone-600">{card.category}</span>
        <span className="text-xs text-stone-400">#{String(card.id).padStart(3, '0')}</span>
      </div>
      ...
      <div className="px-3 py-2 border-b border-black/10 bg-white">
        <h3 className="text-sm font-bold text-stone-900 line-clamp-1">{card.title}</h3>
      </div>
...
function Section({ title, description, children }: { title: string; description: string; children: React.ReactNode }) {
  return (
    <section>
      <div className="mb-6">
        <h2 className="text-sm font-medium tracking-[0.14em] uppercase text-stone-600">{title}</h2>
...
function TierCard({ card }: { card: DemoCard }) {
  return (
    <div className={`shrink-0 w-56 h-72 rounded-md shadow-sm cursor-pointer transition-all duration-300 hover:scale-105 ${tierClass(card.weight || 'default')}`}>
      <CardInner card={card} />
    </div>
  )
}
...
function GradientCard({ card, idx }: { card: DemoCard; idx: number }) {
  ...
  return (
    <div className={`shrink-0 w-56 h-72 rounded-lg cursor-pointer transition-all duration-500 gradient-card gradient-${tier}`}>
      <div className="h-full flex flex-col overflow-hidden rounded-lg bg-white m-[2px]">
        <div className="px-3 py-2 flex items-center justify-between">
          <span className="text-xs font-bold uppercase tracking-widest" style={{ color: card.accentColor }}>{tier}</span>
          <span className="text-xs text-stone-400">#{String(card.id).padStart(3, '0')}</span>
        </div>
        <div className="relative h-28 bg-stone-100 overflow-hidden flex items-center justify-center">
          {card.thumbnail ? <img src={card.thumbnail} alt={card.title} className="w-full h-full object-cover" /> : (
            <div className="w-12 h-12 rounded-full flex items-center justify-center text-xl font-bold text-white" style={{ background: card.accentColor }}>{card.title.charAt(0)}</div>
          )}
        </div>
        <div className="px-3 py-2 bg-white"><h3 className="text-sm font-bold text-stone-900 line-clamp-1">{card.title}</h3></div>
        <div className="px-3 py-2 flex-1"><p className="text-xs text-stone-500 line-clamp-2">{card.summary}</p></div>
        <div className="px-3 py-2 border-t border-stone-200/30 flex items-center justify-between">
          <span className="text-xs text-stone-400">{card.platform}</span>
          <span className="text-xs font-medium" style={{ color: card.accentColor }}>{card.type}</span>
        </div>
      </div>
    </div>
  )
...
function GlassCard({ card }: { card: DemoCard }) {
  return (
    <div className="shrink-0 w-56 h-72 rounded-xl cursor-pointer transition-all duration-300 glass-card hover:scale-105 hover:shadow-2xl overflow-hidden"
      style={{ background: 'linear-gradient(135deg, #667eea 0%, #764ba2 50%, #f093fb 100%)' }}>
      <div className="h-full flex flex-col overflow-hidden rounded-xl">
        <div className="px-3 py-2 flex items-center justify-between border-b border-white/10">
          <span className="px-2 py-0.5 text-xs font-medium rounded bg-white/20 text-white backdrop-blur-sm">{card.category}</span>
          <span className="text-xs text-white/60">#{String(card.id).padStart(3, '0')}</span>
        </div>
        <div className="relative h-28 overflow-hidden px-2 py-1">
          {card.thumbnail ? <img src={card.thumbnail} alt={card.title} className="w-full h-full object-cover rounded-md opacity-90" /> : (
            <div className="w-full h-full flex items-center justify-center">
              <div className="w-12 h-12 rounded-full bg-white/20 backdrop-blur flex items-center justify-center text-xl font-bold text-white">{card.title.charAt(0)}</div>
            </div>
          )}
        </div>
        <div className="px-3 py-2 border-b border-white/10"><h3 className="text-sm font-bold text-white line-clamp-1">{card.title}</h3></div>
        <div className="px-3 py-2 flex-1"><p className="text-xs text-white/70 line-clamp-2">{card.summary}</p></div>
  ...
  return (
    <div className="shrink-0 w-56 h-72 rounded-2xl cursor-pointer transition-all duration-300 neu-card hover:scale-[1.03]">
      <div className="h-full flex flex-col overflow-hidden rounded-2xl">
        <div className="px-3 py-2 flex items-center justify-between">
          <span className="px-2 py-0.5 text-xs font-medium rounded-full neu-badge">{card.category}</span>
          <span className="text-xs text-stone-400">#{String(card.id).padStart(3, '0')}</span>
        </div>
        <div className="relative h-28 overflow-hidden px-3 py-1">
          {card.thumbnail ? <img src={card.thumbnail} alt={card.title} className="w-full h-full object-cover rounded-xl" /> : (
            <div className="w-full h-full flex items-center justify-center rounded-xl neu-icon"><div className="w-12 h-12 rounded-full bg-gradient-to-br from-blue-400 to-purple-500 flex items-center justify-center text-xl font-bold text-white shadow-lg">{card.title.charAt(0)}</div></div>
          )}
        </div>
        <div className="px-3 py-2"><h3 className="text-sm font-bold text-stone-700 line-clamp-1">{card.title}</h3></div>
        <div className="px-3 py-1 flex-1"><p className="text-xs text-stone-500 line-clamp-2">{card.summary}</p></div>
        <div className="px-3 py-2 flex items-center justify-between">
          <span className="text-xs text-stone-400">{card.platform}</span>
          <span className="text-xs font-medium text-stone-600">{card.type}</span>
        </div>
    ...
    <div className="shrink-0 w-56 h-72 rounded-lg cursor-pointer transition-all duration-300 holo-card hover:scale-105">
      <div className="h-full flex flex-col overflow-hidden rounded-lg bg-white relative z-10">
        <div className="px-3 py-2 border-b border-stone-200/50 flex items-center justify-between">
          <span className="px-2 py-0.5 text-xs font-bold rounded holo-badge">{card.category}</span>
          <span className="text-xs text-stone-400">#{String(card.id).padStart(3, '0')}</span>
        </div>
        <div className="relative h-28 overflow-hidden px-2">
          {card.thumbnail ? <img src={card.thumbnail} alt={card.title} className="w-full h-full object-cover" /> : (
            <div className="w-full h-full flex items-center justify-center"><div className="w-12 h-12 rounded-full holo-badge flex items-center justify-center text-xl font-bold text-white">{card.title.charAt(0)}</div></div>
          )}
        </div>
        <div className="px-3 py-2 border-b border-stone-200/50"><h3 className="text-sm font-bold text-stone-900 line-clamp-1">{card.title}</h3></div>
        <div className="px-3 py-2 flex-1"><p className="text-xs text-stone-500 line-clamp-2">{card.summary}</p></div>
...
function NeonCard({ card, idx }: { card: DemoCard; idx: number }) {
  ...
  return (
    <div className={`shrink-0 w-56 h-72 rounded-lg cursor-pointer transition-all duration-300 hover:scale-105 neon-card-${neon.key}`}>
      <div className="h-full flex flex-col overflow-hidden rounded-lg">
        <div className="px-3 py-2 flex items-center justify-between border-b" style={{ borderColor: neon.color + '30' }}>
          <span className="px-2 py-0.5 text-xs font-bold rounded uppercase tracking-wider neon-text" style={{ color: neon.color }}>{card.category}</span>
          <span className="text-xs text-stone-500">#{String(card.id).padStart(3, '0')}</span>
        </div>
        <div className="relative h-28 overflow-hidden px-2 py-1 flex items-center justify-center">
          {card.thumbnail ? <img src={card.thumbnail} alt={card.title} className="w-full h-full object-cover rounded opacity-80" /> : (
            <div className="w-12 h-12 rounded-full flex items-center justify-center text-xl font-bold neon-icon" style={{ color: neon.color, border: `2px solid ${neon.color}`, boxShadow: `0 0 15px ${neon.color}40, inset 0 0 15px ${neon.color}20` }}>{card.title.charAt(0)}</div>
          )}
        </div>
        <div className="px-3 py-2"><h3 className="text-sm font-bold text-stone-100 line-clamp-1">{card.title}</h3></div>
        <div className="px-3 py-1 flex-1"><p className="text-xs text-stone-400 line-clamp-2">{card.summary}</p></div>
        <div className="px-3 py-2 border-t flex items-center justify-between" style={{ borderColor: neon.color + '20' }}>
          <span className="text-xs text-stone-500">{card.platform}</span>
          <span className="text-xs font-medium" style={{ color: neon.color }}>{card.type}</span>
        </div>
      </div>
    </div>
  )
...
function BrutalistCard({ card, idx }: { card: DemoCard; idx: number }) {
  ...
  return (
    <div className={`shrink-0 w-56 h-72 cursor-pointer transition-all duration-200 brutalist-card`} style={{ border: '3px solid #000', boxShadow: '6px 6px 0px #000' }}>
      <div className="h-full flex flex-col overflow-hidden bg-white">
        <div className="px-3 py-2 border-b-2 border-black flex items-center justify-between">
          <span className="px-2 py-0.5 text-xs font-black uppercase tracking-wider text-white" style={{ background: color }}>{card.category}</span>
          <span className="text-xs font-mono font-bold">#{String(card.id).padStart(3, '0')}</span>
        </div>
        <div className="relative h-28 overflow-hidden bg-stone-100 border-b-2 border-black flex items-center justify-center">
          {card.thumbnail ? <img src={card.thumbnail} alt={card.title} className="w-full h-full object-cover" /> : (
            <div className="w-14 h-14 flex items-center justify-center text-2xl font-black text-white" style={{ background: color }}>{card.title.charAt(0)}</div>
          )}
        </div>
        <div className="px-3 py-2 border-b-2 border-black bg-white"><h3 className="text-sm font-black text-black uppercase line-clamp-1">{card.title}</h3></div>
        <div className="px-3 py-2 flex-1"><p className="text-xs text-stone-700 line-clamp-2 font-mono">{card.summary}</p></div>
        <div className="px-3 py-2 border-t-2 border-black flex items-center justify-between">
          <span className="text-xs font-bold text-black uppercase">{card.platform}</span>
          <span className="text-xs font-bold" style={{ color }}>{card.type}</span>
        </div>
      </div>
    </div>
  )
...
function AuroraCard({ card, idx }: { card: DemoCard; idx: number }) {
  return (
    <div className="shrink-0 w-56 h-72 rounded-2xl cursor-pointer transition-all duration-500 hover:scale-105 aurora-card overflow-hidden" style={{ background: auroraGradients[idx % auroraGradients.length] }}>
      <div className="h-full flex flex-col overflow-hidden rounded-2xl backdrop-blur-sm bg-white/5">
        <div className="px-3 py-2 flex items-center justify-between border-b border-white/10">
          <span className="px-2 py-0.5 text-xs font-semibold rounded-full bg-white/15 text-white backdrop-blur-md">{card.category}</span>
          <span className="text-xs text-white/50">#{String(card.id).padStart(3, '0')}</span>
        </div>
        <div className="relative h-28 overflow-hidden px-2 py-1 flex items-center justify-center">
          {card.thumbnail ? <img src={card.thumbnail} alt={card.title} className="w-full h-full object-cover rounded-xl opacity-85" /> : (
            <div className="w-12 h-12 rounded-full bg-white/15 backdrop-blur flex items-center justify-center text-xl font-bold text-white border border-white/20">{card.title.charAt(0)}</div>
          )}
        </div>
        <div className="px-3 py-2 border-b border-white/10"><h3 className="text-sm font-bold text-white line-clamp-1">{card.title}</h3></div>
        <div className="px-3 py-1 flex-1"><p className="text-xs text-white/65 line-clamp-2">{card.summary}</p></div>
        <div className="px-3 py-2 border-t border-white/10 flex items-center justify-between">
...
function RetroCard({ card, idx }: { card: DemoCard; idx: number }) {
  ...
  return (
    <div className={`shrink-0 w-56 h-72 cursor-pointer transition-all duration-300 retro-card retro-${accent}`}>
      <div className="h-full flex flex-col overflow-hidden font-mono">
        <div className="px-3 py-2 flex items-center justify-between border-b" style={{ borderColor: color + '40' }}>
          <span className="text-xs font-bold uppercase tracking-widest" style={{ color, textShadow: `0 0 6px ${color}80` }}>{'>'} {card.category}</span>
          <span className="text-xs" style={{ color: color + '60' }}>#{String(card.id).padStart(3, '0')}</span>
        </div>
        <div className="relative h-28 overflow-hidden flex items-center justify-center border-b" style={{ borderColor: color + '20' }}>
          <div className="text-6xl font-black" style={{ color: color + '30' }}>{card.title.charAt(0)}</div>
        </div>
        <div className="px-3 py-2"><h3 className="text-sm font-bold line-clamp-1" style={{ color }}>{card.title}</h3></div>
        <div className="px-3 py-1 flex-1"><p className="text-xs line-clamp-2" style={{ color: color + '80' }}>{card.summary}</p></div>
        <div className="px-3 py-2 border-t flex items-center justify-between" style={{ borderColor: color + '30' }}>
          <span className="text-xs" style={{ color: color + '60' }}>{card.platform}</span>
          <span className="text-xs blink-cursor" style={{ color }}>{card.type}_</span>
        </div>
      </div>
    </div>
  )
...
function WatercolorCard({ card, idx }: { card: DemoCard; idx: number }) {
  return (
    <div className="shrink-0 w-56 h-72 rounded-3xl cursor-pointer transition-all duration-500 watercolor-card hover:scale-105 overflow-hidden" style={{ background: watercolorBgs[idx % watercolorBgs.length] }}>
      <div className="h-full flex flex-col overflow-hidden rounded-3xl">
        <div className="px-4 py-2 flex items-center justify-between">
          <span className="px-2 py-0.5 text-xs font-semibold rounded-full bg-white/40 text-stone-700 backdrop-blur-sm">{card.category}</span>
          <span className="text-xs text-stone-500/60">#{String(card.id).padStart(3, '0')}</span>
        </div>
        <div className="relative h-28 overflow-hidden px-4 py-1 flex items-center justify-center">
          <div className="w-16 h-16 rounded-full flex items-center justify-center text-2xl font-light text-white" style={{ background: watercolorDotBgs[idx % watercolorDotBgs.length] }}>{card.title.charAt(0)}</div>
        </div>
        <div className="px-4 py-2"><h3 className="text-sm font-semibold text-stone-800 line-clamp-1" style={{ fontFamily: 'Georgia, serif' }}>{card.title}</h3></div>
        <div className="px-4 py-1 flex-1"><p className="text-xs text-stone-600/80 line-clamp-2" style={{ fontFamily: 'Georgia, serif' }}>{card.summary}</p></div>
        <div className="px-4 py-2 flex items-center justify-between">
          <span className="text-xs text-stone-500/70" style={{ fontFamily: 'Georgia, serif' }}>{card.platform}</span>
          <span className="text-xs font-medium text-stone-600" style={{ fontFamily: 'Georgia, serif' }}>{card.type}</span>
        </div>
      </div>
    </div>
  )
...
function MeshCard({ card, idx }: { card: DemoCard; idx: number }) {
  return (
    <div className="shrink-0 w-56 h-72 rounded-2xl cursor-pointer transition-all duration-500 hover:scale-105 mesh-card overflow-hidden" style={{ background: meshGradients[idx % meshGradients.length] }}>
      <div className="h-full flex flex-col overflow-hidden rounded-2xl backdrop-blur-xs">
        <div className="px-3 py-2 flex items-center justify-between border-b border-white/15">
          <span className="px-2 py-0.5 text-xs font-bold rounded bg-black/20 text-white backdrop-blur uppercase tracking-wider">{card.category}</span>
          <span className="text-xs text-white/40">#{String(card.id).padStart(3, '0')}</span>
        </div>
        <div className="relative h-28 overflow-hidden px-2 py-1 flex items-center justify-center">
          <div className="w-14 h-14 rounded-2xl bg-white/15 backdrop-blur-md flex items-center justify-center text-2xl font-bold text-white border border-white/20 shadow-lg">{card.title.charAt(0)}</div>
        </div>
        <div className="px-3 py-2 border-b border-white/10"><h3 className="text-sm font-bold text-white line-clamp-1 drop-shadow-sm">{card.title}</h3></div>
        <div className="px-3 py-1 flex-1"><p className="text-xs text-white/60 line-clamp-2">{card.summary}</p></div>
        <div className="px-3 py-2 border-t border-white/10 flex items-center justify-between">
    ...
    <div className="shrink-0 w-56 h-72 cursor-pointer transition-all duration-300 sketch-card hover:rotate-1 hover:scale-105">
      <div className="h-full flex flex-col overflow-hidden">
        <div className="px-3 py-2 flex items-center justify-between border-b-2 border-dashed border-stone-400">
          <span className="px-2 py-0.5 text-xs font-bold rounded-sm bg-yellow-100 text-stone-700 sketch-font">{card.category}</span>
          <span className="text-xs text-stone-400 sketch-font">#{String(card.id).padStart(3, '0')}</span>
        </div>
        <div className="relative h-28 overflow-hidden flex items-center justify-center border-b-2 border-dashed border-stone-300">
          <svg width="64" height="64" viewBox="0 0 64 64">
            <circle cx="32" cy="32" r="28" fill="none" stroke="#aaa" strokeWidth="2" strokeDasharray="6,3" />
            <text x="32" y="38" textAnchor="middle" fill="#888" fontSize="22" fontFamily="Comic Sans MS, cursive">{card.title.charAt(0)}</text>
          </svg>
        </div>
        <div className="px-3 py-2 border-b border-dashed border-stone-200"><h3 className="text-sm font-bold text-stone-700 line-clamp-1 sketch-font">{card.title}</h3></div>
        <div className="px-3 py-2 flex-1"><p className="text-xs text-stone-500 line-clamp-2 sketch-font">{card.summary}</p></div>
        <div className="px-3 py-2 flex items-center justify-between border-t-2 border-dashed border-stone-400">
          <span className="text-xs text-stone-400 sketch-font">{card.platform}</span>
          <span className="text-xs font-bold text-stone-600 sketch-font">{card.type}</span>
        </div>
      </div>
    </div>
  )
...
function MetallicCard({ card, idx }: { card: DemoCard; idx: number }) {
  ...
  return (
    <div className={`shrink-0 w-56 h-72 rounded-lg cursor-pointer transition-all duration-300 hover:scale-105 metallic-${type}`}>
      <div className="h-full flex flex-col overflow-hidden rounded-lg metallic-inner">
        <div className="px-3 py-2 flex items-center justify-between border-b border-white/10">
          <span className="px-2 py-0.5 text-xs font-bold rounded uppercase tracking-wider" style={metallicBadge(type)}>{card.category}</span>
          <span className="text-xs" style={{ color: metallicTextSub(type) }}>#{String(card.id).padStart(3, '0')}</span>
        </div>
        <div className="relative h-28 overflow-hidden px-2 py-1 flex items-center justify-center">
          <div className="w-14 h-14 rounded-full flex items-center justify-center text-2xl font-black metallic-icon" style={metallicIconStyle(type)}>{card.title.charAt(0)}</div>
        </div>
        <div className="px-3 py-2 border-b border-white/10"><h3 className="text-sm font-bold line-clamp-1" style={{ color: metallicTextMain(type) }}>{card.title}</h3></div>
        <div className="px-3 py-1 flex-1"><p className="text-xs line-clamp-2" style={{ color: metallicTextSub(type) }}>{card.summary}</p></div>
        <div className="px-3 py-2 border-t border-white/10 flex items-center justify-between">
          <span className="text-xs" style={{ color: metallicTextSub(type) }}>{card.platform}</span>
          <span className="text-xs font-bold" style={{ color: metallicTextMain(type) }}>{card.type}</span>
        </div>
      </div>
    </div>
  )
...
function PixelCard({ card, idx }: { card: DemoCard; idx: number }) {
  ...
  return (
    <div className="shrink-0 w-56 h-72 cursor-pointer transition-all duration-200 pixel-card hover:translate-y-[-4px]" style={{ border: `4px solid ${color}`, boxShadow: `4px 4px 0px ${color}80` }}>
      <div className="h-full flex flex-col overflow-hidden pixel-font" style={{ fontFamily: "'Courier New', monospace", imageRendering: 'pixelated' }}>
        <div className="px-3 py-2 flex items-center justify-between border-b-4" style={{ borderColor: color + '60', background: color + '15' }}>
          <span className="text-xs font-bold uppercase" style={{ color }}>{card.category}</span>
          <span className="text-xs text-stone-400">#{String(card.id).padStart(3, '0')}</span>
        </div>
        <div className="relative h-28 overflow-hidden flex items-center justify-center bg-stone-900">
          <div className="text-5xl font-black pixel-bounce" style={{ color, textShadow: `3px 3px 0px ${color}40` }}>{card.title.charAt(0)}</div>
        </div>
        <div className="px-3 py-2 bg-stone-900"><h3 className="text-sm font-bold text-stone-100 line-clamp-1">{card.title}</h3></div>
        <div className="px-3 py-1 flex-1 bg-stone-900"><p className="text-xs text-stone-400 line-clamp-2">{card.summary}</p></div>
        <div className="px-3 py-2 border-t-4 flex items-center justify-between bg-stone-900" style={{ borderColor: color + '40' }}>
          <span className="text-xs text-stone-500">{card.platform}</span>
          <span className="text-xs font-bold" style={{ color }}>{card.type}</span>
        </div>
      </div>
...
function PapercutCard({ card, idx }: { card: DemoCard; idx: number }) {
  ...
  return (
    <div className={`shrink-0 w-56 h-72 cursor-pointer transition-all duration-300 papercut-card papercut-${colorKey}`}>
      <div className="h-full flex flex-col overflow-hidden rounded-sm bg-white relative z-10">
        <div className="px-3 py-2 flex items-center justify-between border-b" style={{ borderColor: color + '40' }}>
          <span className="text-xs font-bold uppercase tracking-wider" style={{ color }}>{card.category}</span>
          <span className="text-xs text-stone-400">#{String(card.id).padStart(3, '0')}</span>
        </div>
        <div className="relative h-28 overflow-hidden flex items-center justify-center bg-stone-50">
          {card.thumbnail ? <img src={card.thumbnail} alt={card.title} className="w-full h-full object-cover" /> : (
            <div className="w-14 h-14 rounded-full flex items-center justify-center text-2xl font-bold text-white" style={{ background: color }}>{card.title.charAt(0)}</div>
          )}
        </div>
        <div className="px-3 py-2"><h3 className="text-sm font-bold text-stone-800 line-clamp-1">{card.title}</h3></div>
        <div className="px-3 py-1 flex-1"><p className="text-xs text-stone-500 line-clamp-2">{card.summary}</p></div>
        <div className="px-3 py-2 border-t border-stone-200 flex items-center justify-between">
          <span className="text-xs text-stone-400">{card.platform}</span>
          <span className="text-xs font-medium" style={{ color }}>{card.type}</span>
        </div>
      </div>
    </div>
  )
...
function VaporwaveCard({ card, idx }: { card: DemoCard; idx: number }) {
  return (
    <div className="shrink-0 w-56 h-72 rounded-lg cursor-pointer transition-all duration-300 vaporwave-card overflow-hidden" style={{ background: vaporwaveBgs[idx % vaporwaveBgs.length] }}>
      <div className="h-full flex flex-col overflow-hidden rounded-lg relative">
        <div className="px-3 py-2 flex items-center justify-between border-b border-pink-300/30">
          <span className="text-xs font-bold uppercase tracking-widest text-cyan-300" style={{ textShadow: '0 0 8px #00ffff80' }}>{card.category}</span>
          <span className="text-xs text-pink-300/60">#{String(card.id).padStart(3, '0')}</span>
        </div>
        <div className="relative h-28 overflow-hidden flex items-center justify-center">
          <span className="text-5xl font-black text-transparent bg-clip-text" style={{ backgroundImage: 'linear-gradient(180deg, #ff71ce, #01cdfe, #05ffa1)', WebkitBackgroundClip: 'text' }}>{card.title.charAt(0)}</span>
        </div>
        <div className="px-3 py-2"><h3 className="text-sm font-bold text-pink-200 line-clamp-1">{card.title}</h3></div>
        <div className="px-3 py-1 flex-1"><p className="text-xs text-purple-300/70 line-clamp-2">{card.summary}</p></div>
...
function NewspaperCard({ card }: { card: DemoCard }) {
  return (
    <div className="shrink-0 w-56 h-72 cursor-pointer transition-all duration-300 newspaper-card">
      <div className="h-full flex flex-col overflow-hidden" style={{ fontFamily: "'Times New Roman', Georgia, serif" }}>
        <div className="px-3 py-2 flex items-center justify-between border-b-2 border-black">
          <span className="text-xs font-bold uppercase tracking-[0.2em]">{card.category}</span>
          <span className="text-xs text-stone-500 italic">No.{String(card.id).padStart(3, '0')}</span>
        </div>
        <div className="relative h-28 overflow-hidden flex items-center justify-center border-b border-stone-300">
          <span className="text-6xl font-black text-stone-800" style={{ fontFamily: "'Playfair Display', Georgia, serif" }}>{card.title.charAt(0)}</span>
        </div>
        <div className="px-3 py-2 border-b border-stone-300"><h3 className="text-sm font-bold text-stone-900 line-clamp-1 leading-tight">{card.title}</h3></div>
        <div className="px-3 py-1 flex-1"><p className="text-xs text-stone-600 line-clamp-3 leading-relaxed" style={{ textAlign: 'justify' }}>{card.summary}</p></div>
        <div className="px-3 py-2 border-t-2 border-black flex items-center justify-between">
...
function FrostedCard({ card, idx }: { card: DemoCard; idx: number }) {
  ...
  return (
    <div className="shrink-0 w-56 h-72 rounded-xl cursor-pointer transition-all duration-300 frosted-card" style={{ '--frost-accent': accent } as React.CSSProperties}>
      <div className="h-full flex flex-col overflow-hidden rounded-xl">
        <div className="px-3 py-2 flex items-center justify-between border-b border-white/5">
          <span className="text-xs font-bold uppercase tracking-wider" style={{ color: accent }}>{card.category}</span>
          <span className="text-xs text-stone-500">#{String(card.id).padStart(3, '0')}</span>
        </div>
        <div className="relative h-28 overflow-hidden flex items-center justify-center">
          <div className="w-14 h-14 rounded-xl flex items-center justify-center text-2xl font-bold text-white/90" style={{ background: accent + '30', border: `1px solid ${accent}40` }}>{card.title.charAt(0)}</div>
        </div>
        <div className="px-3 py-2"><h3 className="text-sm font-bold text-stone-200 line-clamp-1">{card.title}</h3></div>
        <div className="px-3 py-1 flex-1"><p className="text-xs text-stone-400 line-clamp-2">{card.summary}</p></div>
        <div className="px-3 py-2 border-t border-white/5 flex items-center justify-between">
          <span className="text-xs text-stone-500">{card.platform}</span>
          <span className="text-xs font-medium" style={{ color: accent }}>{card.type}</span>
        </div>
      </div>
    </div>
  )
...
function OrigamiCard({ card, idx }: { card: DemoCard; idx: number }) {
  ...
  return (
    <div className="shrink-0 w-56 h-72 cursor-pointer transition-all duration-300 origami-card" style={{ '--origami-color': color } as React.CSSProperties}>
      <div className="h-full flex flex-col overflow-hidden bg-white relative z-10">
        ...
        <div className="relative h-28 overflow-hidden flex items-center justify-center bg-stone-50">
          <div className="w-12 h-12 flex items-center justify-center text-xl font-bold text-white" style={{ background: color, clipPath: 'polygon(50% 0%, 100% 50%, 50% 100%, 0% 50%)' }}>{card.title.charAt(0)}</div>
        </div>
        <div className="px-3 py-2 border-b border-stone-100"><h3 className="text-sm font-bold text-stone-800 line-clamp-1">{card.title}</h3></div>
        <div className="px-3 py-1 flex-1"><p className="text-xs text-stone-500 line-clamp-2">{card.summary}</p></div>
        <div className="px-3 py-2 border-t border-stone-200 flex items-center justify-between">
          <span className="text-xs text-stone-400">{card.platform}</span>
          <span className="text-xs font-medium" style={{ color }}>{card.type}</span>
...
function GlitchCard({ card, idx }: { card: DemoCard; idx: number }) {
  ...
  return (
    <div className="shrink-0 w-56 h-72 cursor-pointer transition-all duration-300 glitch-card" style={{ '--glitch-color': color } as React.CSSProperties}>
      <div className="h-full flex flex-col overflow-hidden rounded-sm bg-stone-900 relative">
        <div className="px-3 py-2 flex items-center justify-between border-b border-stone-800">
          <span className="text-xs font-bold uppercase tracking-widest font-mono glitch-text" style={{ color }}>{card.category}</span>
          <span className="text-xs text-stone-600 font-mono">#{String(card.id).padStart(3, '0')}</span>
        </div>
        <div className="relative h-28 overflow-hidden flex items-center justify-center bg-black">
          <span className="text-5xl font-black font-mono glitch-text" style={{ color, textShadow: '2px 0 #ff0000, -2px 0 #00ffff' }}>{card.title.charAt(0)}</span>
        </div>
        <div className="px-3 py-2 bg-stone-900"><h3 className="text-sm font-bold text-stone-200 line-clamp-1 font-mono">{card.title}</h3></div>
        <div className="px-3 py-1 flex-1 bg-stone-900"><p className="text-xs text-stone-500 line-clamp-2 font-mono">{card.summary}</p></div>
        <div className="px-3 py-2 border-t border-stone-800 flex items-center justify-between bg-stone-900">
          <span className="text-xs text-stone-600 font-mono">{card.platform}</span>
          <span className="text-xs font-bold font-mono" style={{ color }}>{card.type}</span>
        </div>
      </div>
    </div>
  )
...
function LiveResourceCard({ resource, style }: { resource: DbResource; style: CardStyleEntry }) {
  ...
  const renderContent = () => (
    <>
      <div className="px-3 py-2 flex items-center justify-between">
        <span className="text-xs font-bold uppercase tracking-wider" style={{ color: color || '#6b7280' }}>{resource.category_name || resource.platform}</span>
        <span className="text-xs text-stone-400">#{String(resource.id).padStart(3, '0')}</span>
      </div>
      <div className="relative h-28 bg-stone-100 overflow-hidden flex items-center justify-center">
        {resource.thumbnail ? <img src={resource.thumbnail} alt={resource.title} className="w-full h-full object-cover" /> : (
          <div className="w-12 h-12 rounded-full flex items-center justify-center text-xl font-bold text-white" style={{ background: color || '#6b7280' }}>{(resource.title || 'R').charAt(0)}</div>
        )}
      </div>
      <div className="px-3 py-2 bg-white"><h3 className="text-sm font-bold text-stone-900 line-clamp-1">{resource.title}</h3></div>
      <div className="px-3 py-2 flex-1 bg-stone-50"><p className="text-xs text-stone-500 line-clamp-2">{resource.summary || 'No description'}</p></div>
      <div className="px-3 py-2 border-t border-stone-200 flex items-center justify-between">
        <span className="text-xs text-stone-400">{resource.platform}</span>
        <span className="text-xs font-medium" style={{ color: color || '#6b7280' }}>{resource.resource_type}</span>
      </div>
    </>
  )
  ...
  if (style.style === 'tier') {
    return (
      <div className={`w-full h-72 rounded-md cursor-pointer transition-all duration-300 hover:scale-105 ${tierClass(style.variant)}`}>
        <div className="h-full flex flex-col overflow-hidden rounded-md">{renderContent()}</div>
      </div>
    )
  }
  ...
  if (style.style === 'gradient') {
    return (
      <div className={`w-full h-72 rounded-lg cursor-pointer transition-all duration-500 gradient-card gradient-${style.variant}`}>
        <div className="h-full flex flex-col overflow-hidden rounded-lg bg-white m-[2px]">{renderContent()}</div>
      </div>
    )
  }
  ...
  if (style.style === 'glass') {
    return (
      <div className="w-full h-72 rounded-xl cursor-pointer transition-all duration-300 glass-card hover:scale-105 overflow-hidden" style={{ background: bg }}>
        <div className="h-full flex flex-col overflow-hidden rounded-xl backdrop-blur-md bg-white/8">{renderContent()}</div>
      </div>
    )
  }
  ...
  if (style.style === 'neu') {
    return (
      <div className="w-full h-72 rounded-2xl cursor-pointer transition-all duration-300 neu-card hover:scale-[1.03]">
        <div className="h-full flex flex-col overflow-hidden rounded-2xl">{renderContent()}</div>
      </div>
    )
  }
  ...
  if (style.style === 'holo') {
    return (
      <div className="w-full h-72 rounded-lg cursor-pointer transition-all duration-300 holo-card hover:scale-105">
        <div className="h-full flex flex-col overflow-hidden rounded-lg bg-white relative z-10">{renderContent()}</div>
      </div>
    )
  }
  ...
  if (style.style === 'neon') {
    return (
      <div className={`w-full h-72 rounded-lg cursor-pointer transition-all duration-300 hover:scale-105 neon-card-${style.variant}`}>
        <div className="h-full flex flex-col overflow-hidden rounded-lg">{renderContent()}</div>
      </div>
    )
  }
  ...
  if (style.style === 'brutalist') {
    return (
      <div className="w-full h-72 cursor-pointer transition-all duration-200 brutalist-card" style={{ border: '3px solid #000', boxShadow: '6px 6px 0px #000' }}>
        <div className="h-full flex flex-col overflow-hidden bg-white">{renderContent()}</div>
      </div>
    )
  }
  ...
  if (style.style === 'aurora') {
    return (
      <div className="w-full h-72 rounded-2xl cursor-pointer transition-all duration-500 hover:scale-105 aurora-card overflow-hidden" style={{ background: bg }}>
        <div className="h-full flex flex-col overflow-hidden rounded-2xl backdrop-blur-sm bg-white/5">{renderContent()}</div>
      </div>
    )
  }
  ...
  if (style.style === 'retro') {
    return (
      <div className={`w-full h-72 cursor-pointer transition-all duration-300 retro-card overflow-hidden retro-${style.variant}`}>
        <div className="h-full flex flex-col overflow-hidden font-mono">{renderContent()}</div>
      </div>
    )
  }
  ...
  if (style.style === 'watercolor') {
    return (
      <div className="w-full h-72 rounded-2xl cursor-pointer transition-all duration-500 hover:scale-105 watercolor-card overflow-hidden" style={{ background: watercolorBgs[0] }}>
        <div className="h-full flex flex-col overflow-hidden rounded-2xl">{renderContent()}</div>
      </div>
    )
  }
  ...
  if (style.style === 'mesh') {
    return (
      <div className="w-full h-72 rounded-xl cursor-pointer transition-all duration-500 hover:scale-105 mesh-card overflow-hidden" style={{ background: meshGradients[0] }}>
        <div className="h-full flex flex-col overflow-hidden rounded-xl">{renderContent()}</div>
      </div>
    )
  }
  ...
  if (style.style === 'sketch') {
    return (
      <div className="w-full h-72 cursor-pointer transition-all duration-300 sketch-card hover:scale-[1.03] overflow-hidden">
        <div className="h-full flex flex-col overflow-hidden">{renderContent()}</div>
      </div>
    )
  }
  ...
  if (style.style === 'metallic') {
    return (
      <div className={`w-full h-72 rounded-lg cursor-pointer transition-all duration-300 hover:scale-105 metallic-${style.variant}`}>
        <div className="h-full flex flex-col overflow-hidden rounded-lg">{renderContent()}</div>
      </div>
    )
  }
  ...
  if (style.style === 'pixel') {
    return (
      <div className="w-full h-72 cursor-pointer transition-all duration-200 pixel-card overflow-hidden" style={{ borderColor: color }}>
        <div className="h-full flex flex-col overflow-hidden">{renderContent()}</div>
      </div>
    )
  }
  ...
  if (style.style === 'papercut') {
    return (
      <div className={`w-full h-72 cursor-pointer transition-all duration-300 papercut-card papercut-${style.variant}`}>
        <div className="h-full flex flex-col overflow-hidden rounded-sm bg-white relative z-10">{renderContent()}</div>
      </div>
    )
  }
  ...
  if (style.style === 'vaporwave') {
    return (
      <div className="w-full h-72 rounded-lg cursor-pointer transition-all duration-300 vaporwave-card overflow-hidden" style={{ background: vaporwaveBgs[0] }}>
        <div className="h-full flex flex-col overflow-hidden rounded-lg relative">{renderContent()}</div>
      </div>
    )
  }
  ...
  if (style.style === 'frosted') {
    return (
      <div className="w-full h-72 rounded-xl cursor-pointer transition-all duration-300 frosted-card overflow-hidden" style={{ '--frost-accent': color } as React.CSSProperties}>
        <div className="h-full flex flex-col overflow-hidden rounded-xl">{renderContent()}</div>
      </div>
    )
  }
  ...
  if (style.style === 'origami') {
    return (
      <div className="w-full h-72 cursor-pointer transition-all duration-300 origami-card overflow-hidden" style={{ '--origami-color': color } as React.CSSProperties}>
        <div className="h-full flex flex-col overflow-hidden bg-white relative z-10">{renderContent()}</div>
      </div>
    )
  }
  ...
  if (style.style === 'glitch') {
    return (
      <div className="w-full h-72 cursor-pointer transition-all duration-300 glitch-card overflow-hidden" style={{ '--glitch-color': color } as React.CSSProperties}>
        <div className="h-full flex flex-col overflow-hidden rounded-sm bg-stone-900 relative">{renderContent()}</div>
      </div>
    )
  }
...
export default function CardUI() {
  ...
  const [cardStyleMap, setCardStyleMap] = useState<Record<number, CardStyleEntry>>({})
  ...
  return (
    <div className="min-h-screen bg-stone-50">
      ...
      <div className="mx-auto max-w-7xl px-4 py-8 space-y-12">
        ...
        <Section title="Weight Tier System" description="Gradient border effects ranked by importance level">
          {tierCards.map((card) => <TierCard key={card.id} card={card} />)}
        </Section>

        {/* Section 2: Gradient Border */}
        <Section title="Gradient Border Effects" description="Animated gradient borders with glow effects">
          {gradientCards.map((card, idx) => <GradientCard key={card.id} card={card} idx={idx} />)}
        </Section>

        {/* Section 3: Glassmorphism */}
        <Section title="Glassmorphism" description="Frosted glass effect with translucent layers">
          {glassCards.map((card) => <GlassCard key={card.id} card={card} />)}
        </Section>
        ...
        <Section title="Neumorphism" description="Soft UI with embossed depth">
          <div className="rounded-xl p-8 bg-stone-200">
            <div className="flex flex-wrap gap-6">
              {neuCards.map((card) => <NeuCard key={card.id} card={card} />)}
            </div>
          </div>
        </Section>

        {/* Section 5: Holographic */}
        <Section title="Holographic" description="Rainbow shimmer with prismatic light effects">
          {holoCards.map((card) => <HoloCard key={card.id} card={card} />)}
        </Section>

        {/* Section 6: Cyberpunk Neon */}
        <Section title="Cyberpunk Neon" description="Dark theme with neon glow accents">
          <div className="rounded-xl p-8 bg-stone-950">
            <div className="flex flex-wrap gap-6">
              {neonCards.map((card, idx) => <NeonCard key={card.id} card={card} idx={idx} />)}
            </div>
          </div>
        </Section>

        {/* Section 7: Brutalist */}
        <Section title="Brutalist" description="Raw, bold, unapologetic design with hard shadows">
          {brutalistCards.map((card, idx) => <BrutalistCard key={card.id} card={card} idx={idx} />)}
        </Section>

        {/* Section 8: Aurora */}
        <Section title="Aurora Liquid" description="Flowing gradients with Northern Lights atmosphere">
          {auroraCards.map((card, idx) => <AuroraCard key={card.id} card={card} idx={idx} />)}
        </Section>

        {/* Section 9: Retro Terminal */}
        <Section title="Retro Terminal" description="CRT scanline aesthetic with phosphor glow">
          <div className="rounded-xl p-8 bg-stone-950">
            <div className="flex flex-wrap gap-6">
              {retroCards.map((card, idx) => <RetroCard key={card.id} card={card} idx={idx} />)}
            </div>
          </div>
        </Section>

        {/* Section 10: Watercolor */}
        <Section title="Watercolor" description="Soft ink wash with organic edges and translucent layers">
          {watercolorCards.map((card, idx) => <WatercolorCard key={card.id} card={card} idx={idx} />)}
        </Section>

        {/* Section 11: Mesh Gradient */}
        <Section title="Mesh Gradient" description="Multi-point color mesh with organic blob shapes">
          {meshCards.map((card, idx) => <MeshCard key={card.id} card={card} idx={idx} />)}
        </Section>

        {/* Section 12: Sketch */}
        <Section title="Sketch / Wireframe" description="Hand-drawn pencil sketch aesthetic with wobbly borders">
          {sketchCards.map((card) => <SketchCard key={card.id} card={card} />)}
        </Section>

        {/* Section 13: Metallic */}
        <Section title="Metallic" description="Brushed steel and chrome with reflective surfaces">
          {metallicCards.map((card, idx) => <MetallicCard key={card.id} card={card} idx={idx} />)}
        </Section>

        {/* Section 14: Pixel Art */}
        <Section title="Pixel Art / 8-bit" description="Retro gaming pixel aesthetic with chunky borders">
          <div className="rounded-xl p-8 bg-stone-900">
            <div className="flex flex-wrap gap-6">
              {pixelCards.map((card, idx) => <PixelCard key={card.id} card={card} idx={idx} />)}
            </div>
          </div>
        </Section>

        {/* Section 15: Paper Cut */}
        <Section title="Paper Cut / Layered" description="Multi-layer paper shadow effect with pastel tones">
          <div className="rounded-xl bg-stone-100 p-8">
            <div className="flex flex-wrap gap-6">
              {papercutCards.map((card, idx) => <PapercutCard key={card.id} card={card} idx={idx} />)}
            </div>
          </div>
        </Section>

        {/* Section 16: Vaporwave */}
        <Section title="Vaporwave / Retrowave" description="80s/90s aesthetic with pink-cyan gradients and grid lines">
          <div className="rounded-xl p-8" style={{ background: 'linear-gradient(180deg, #1a0533 0%, #2d1b69 50%, #ff6ec7 100%)' }}>
            <div className="flex flex-wrap gap-6">
              {vaporwaveCards.map((card, idx) => <VaporwaveCard key={card.id} card={card} idx={idx} />)}
            </div>
          </div>
        </Section>

        {/* Section 17: Newspaper */}
        <Section title="Newspaper / Gazette" description="Classic print media typographic style">
          <div className="rounded-xl p-8 bg-stone-100">
            <div className="flex flex-wrap gap-6">
              {newspaperCards.map((card) => <NewspaperCard key={card.id} card={card} />)}
            </div>
          </div>
        </Section>

        {/* Section 18: Frosted Dark */}
        <Section title="Frosted Dark" description="Dark frosted glass with colored accent borders">
          <div className="rounded-xl p-8" style={{ background: 'linear-gradient(135deg, #0f0f1a 0%, #1a1a2e 50%, #16213e 100%)' }}>
            <div className="flex flex-wrap gap-6">
              {frostedCards.map((card, idx) => <FrostedCard key={card.id} card={card} idx={idx} />)}
            </div>
          </div>
        </Section>

        {/* Section 19: Origami */}
        <Section title="Origami / Fold" description="Paper fold effect with diagonal shadow crease">
          <div className="rounded-xl bg-stone-100 p-8">
            <div className="flex flex-wrap gap-6">
              {origamiCards.map((card, idx) => <OrigamiCard key={card.id} card={card} idx={idx} />)}
            </div>
          </div>
        </Section>

        {/* Section 20: Glitch */}
        <Section title="Glitch / Distorted" description="Digital glitch effect with RGB split and scan artifacts">
          <div className="rounded-xl p-8 bg-stone-950">
            <div className="flex flex-wrap gap-6">
              {glitchCards.map((card, idx) => <GlitchCard key={card.id} card={card} idx={idx} />)}
            </div>
          </div>
        </Section>
        ...
        <section>
          <div className="mb-6 flex items-center justify-between">
            <div>
              <h2 className="text-sm font-medium tracking-[0.14em] uppercase text-stone-600">My Resources — Random Styles</h2>
              <p className="text-sm text-stone-400 mt-1">Resources doubled, each card gets a unique UI style from all 20 styles</p>
            </div>
            <button
          ...
          {loading ? (
            <div className="py-12 text-center">
              ...
              <p className="mt-3 text-sm text-stone-400">Loading resources...</p>
            </div>
          ) : liveResources.length === 0 ? (
            <div className="py-12 text-center">
              <p className="text-sm text-stone-400">No resources found. Add some from the My Resources page.</p>
            </div>
          ) : (
            <div className="grid grid-cols-2 sm:grid-cols-3 md:grid-cols-4 lg:grid-cols-5 gap-5">
              {liveResources.map((r) => (
                cardStyleMap[r.id] ? <LiveResourceCard key={r.id} resource={r} style={cardStyleMap[r.id]} /> : null
              ))}
            </div>
          )}
        </section>
