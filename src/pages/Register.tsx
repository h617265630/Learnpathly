...
export default function Register() {
  ...
  }, [buildFieldErrors, touched.username, touched.email, touched.password, touched.confirm_password])
  ...
  const submit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      setFormError(err?.detail || err?.message || 'Registration failed, please try again')
    } finally {
      setLoading(false)
    }
  }
    ...
    <div className="min-h-[calc(100vh-4rem)] flex">
      ...
      <div className="hidden lg:flex lg:w-1/2 bg-stone-900 relative overflow-hidden flex-col justify-between p-12 xl:p-16">
        ...
        <div className="relative z-10">
          <p className="text-[10px] font-bold uppercase tracking-[0.3em] text-amber-400 mb-6">Learnpathly</p>
        ...
        <div className="relative z-10 space-y-3">
          <div className="flex items-center gap-3">
            ...
            <p className="text-[11px] uppercase tracking-[0.2em] text-stone-500">Free to start · No credit card</p>
          </div>
          <p className="text-xs text-stone-600">Join thousands of learners building structured knowledge</p>
        </div>
      </div>
      ...
      <div className="flex-1 flex flex-col justify-center px-8 py-12 lg:px-16 xl:px-24 bg-stone-50/50">
        ...
        <div className="lg:hidden mb-10">
          ...
          <h1 className="text-3xl font-black text-stone-900 tracking-tight font-serif">Join us.</h1>
        </div>
        ...
        <div className="w-full max-w-sm mx-auto lg:mx-0">
          ...
          <div className="mb-8">
            ...
            <h2 className="text-2xl font-black text-stone-900 font-serif tracking-tight leading-tight">Create account.</h2>
            <p className="mt-2 text-xs text-stone-400">Free to start · No credit card required</p>
          </div>
          ...
          <form onSubmit={submit} className="space-y-5">
            ...
            <div className="space-y-4">
              <div>
                <label htmlFor="username" className="block text-xs font-bold uppercase tracking-[0.15em] text-stone-500 mb-2">Username</label>
                <div className="relative">
                  <div className={`absolute left-0 top-0 bottom-0 w-0.5 transition-all duration-200 ${getInputBorderColor('username')}`} />
                ...
                {fieldErrors.username && (
                  <p id="register-username-error" className="mt-1.5 text-xs text-red-500" role="alert">{fieldErrors.username}</p>
                )}
              </div>

              <div>
                <label htmlFor="email" className="block text-xs font-bold uppercase tracking-[0.15em] text-stone-500 mb-2">Email</label>
                <div className="relative">
                ...
                {fieldErrors.email && (
                  <p id="register-email-error" className="mt-1.5 text-xs text-red-500" role="alert">{fieldErrors.email}</p>
                )}
              </div>
              ...
              <div>
                ...
                {fieldErrors.password && (
                  <p id="register-password-error" className="mt-1.5 text-xs text-red-500" role="alert">{fieldErrors.password}</p>
                )}
              </div>

              <div>
                <label htmlFor="confirmPassword" className="block text-xs font-bold uppercase tracking-[0.15em] text-stone-500 mb-2">Confirm Password</label>
                <div className="relative">
                  <div className={`absolute left-0 top-0 bottom-0 w-0.5 transition-all duration-200 ${getInputBorderColor('confirm_password')}`} />
                  <Lock className="absolute left-3.5 top-1/2 -translate-y-1/2 text-stone-400 w-4 h-4" />
                  <input
                    id="confirmPassword"
                ...
                {fieldErrors.confirm_password && (
                  <p id="register-confirm-error" className="mt-1.5 text-xs text-red-500" role="alert">{fieldErrors.confirm_password}</p>
                )}
              </div>
            </div>
            ...
            <div className="space-y-3 pt-1">
              <div className="flex items-start gap-2.5">
                ...
                />
                <label htmlFor="terms" className="text-xs text-stone-400 leading-relaxed cursor-pointer">
                  I agree to the
                  <Link to="/about" className="text-amber-600 hover:text-amber-700 font-medium"> Terms of Service</Link>
                  and
                  <Link to="/about" className="text-amber-600 hover:text-amber-700 font-medium"> Privacy Policy</Link>
                </label>
              </div>
              ...
              {successMessage && (
                <p className="text-xs text-emerald-600 text-center py-2 border border-emerald-100 bg-emerald-50 rounded font-medium">{successMessage}</p>
              )}
            </div>
          </form>
          ...
          <p className="mt-8 pt-6 border-t border-stone-100 text-center text-xs text-stone-400">
            ...
            <Link to="/login" className="text-amber-600 hover:text-amber-700 font-semibold transition-colors"> Sign in →</Link>
          </p>
        </div>
      </div>
    </div>
