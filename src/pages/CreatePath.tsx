...
const TEMPLATES: Record<TemplateId, { label: string; description: string; meta: Partial<PathMeta> }> = {
  github_trends: {
    ...
    meta: {
      ...
      description: 'Track GitHub Trending weekly: shortlist repos, read READMEs, capture key ideas, and turn them into an actionable learning path.',
  ...
  social_news: {
    ...
    meta: {
      ...
      description: 'Collect tech news, articles, and podcasts you care about: organize by topic, review regularly, and refine over time.',
      type: 'partical pool',
      isPublic: true,
    },
  },
...
export default function CreatePath() {
  ...
  function addResource(resource: UiResource) {
    ...
    if ((!pathMeta.coverImageUrl || isDefaultCover(pathMeta.coverImageUrl)) && firstThumb) {
  ...
  function onDrop(e: React.DragEvent) {
    ...
    if (reorderId && reorderFromStr) {
      ...
      if (Number.isFinite(fromIndex)) moveSelected(fromIndex, selected.length - 1)
      return
    }
  ...
  async function handleSubmit() {
    if (!pathMeta.title.trim() || selected.length === 0 || pathMeta.categoryId == null) return
    setCreateError('')
    setCreating(true)
    try {
  ...
  return (
    <div className="min-h-screen bg-stone-50">
      ...
      <main className="mx-auto max-w-7xl px-4 py-8">
        ...
        <section className="bg-white rounded-md border border-stone-100 p-6 mb-6">
          ...
          <div className="grid grid-cols-12 gap-6">
            ...
            <div className="col-span-12 lg:col-span-7 space-y-5">
              {/* Templates */}
              <div>
                <label className="block text-[11px] font-bold uppercase tracking-widest text-stone-400 mb-2">Templates</label>
                <div className="grid grid-cols-2 gap-2">
                  {(Object.keys(TEMPLATES) as TemplateId[]).map((id) => (
                    ...
                    >
                      <p className="text-xs font-bold text-stone-800">{TEMPLATES[id].label}</p>
                      <p className="text-[10px] text-stone-500 mt-0.5 leading-relaxed">{TEMPLATES[id].description}</p>
                    </button>
                  ))}
                </div>
                <p className="mt-1.5 text-[10px] text-stone-400">Click a template to auto-fill the fields below.</p>
              </div>
                    ...
                    <div className="grid grid-cols-1 gap-2">
                      <button
                        ...
                        className={`rounded-sm overflow-hidden border-2 transition-all ${
                          pathMeta.coverImageUrl === uploadedCoverUrl ? 'border-sky-500' : 'border-stone-200 hover:border-stone-300'
                        }`}
                    ...
                    <div className="grid grid-cols-4 gap-1.5">
                      {defaultCoverUrls.map((u, idx) => (
                        <button
                          ...
                          type="button"
                          className={`rounded-sm overflow-hidden border-2 transition-all ${
                            pathMeta.coverImageUrl === u ? 'border-sky-500' : 'border-stone-200 hover:border-stone-300'
                          }`}
            ...
            <div className="bg-white rounded-md border border-stone-100 p-5 h-full">
              ...
              <div className="max-h-105 overflow-y-auto space-y-2 pr-1 mb-3">
                {filteredResources.map((r) => (
                  <div
                    key={r.id}
                    className="group rounded-sm border border-stone-100 bg-white hover:border-stone-200 hover:shadow-sm transition-all cursor-pointer overflow-hidden"
                    draggable
        ...
        {createError && <p className="text-sm text-red-500 text-right mt-2">{createError}</p>}
      </main>
    </div>
  )
}
